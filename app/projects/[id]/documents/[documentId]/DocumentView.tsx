...
import { DocumentWithTasks, DOCUMENT_TYPES, Task } from '@/lib/types'
...
import {
  ArrowLeft, Edit, Trash2, Link as LinkIcon,
  Clock, User, Download, ExternalLink
} from 'lucide-react'
...
export function DocumentView({ document, projectId, projectName }: DocumentViewProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Delete document_tasks links first
      await supabase
        .from('document_tasks')
        .delete()
        ...
        <Card className="border-moss-100 shadow-sm">
          <CardHeader>
            <CardTitle className="font-display text-lg flex items-center gap-2">
              <LinkIcon className="w-5 h-5 text-moss-600" />
              Tareas vinculadas ({document.tasks.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {document.tasks.map((task: Task) => (
                <Link
                  key={task.id}
                  href={`/projects/${projectId}`}
                  className={cn(
                    'flex items-center justify-between p-3 rounded-lg',
                    'bg-moss-50 hover:bg-moss-100 transition-colors',
                    'group'
                  )}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-sloth-800 group-hover:text-moss-700 transition-colors">
                        {task.title}
                      </p>
                      {task.story_points && (
